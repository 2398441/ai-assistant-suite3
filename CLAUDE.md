...
- **Provider-aware dynamic draft link** — `_notify()` includes `provider` (`"Gmail"` | `"Outlook"` | `"Outlook365"`) and `draft_id` (extracted from the Composio draft creation response) in the SSE event. Domain classification: `gmail.com`/`googlemail.com` → `"Gmail"`; consumer Microsoft domains (`outlook.com`, `hotmail.com`, `live.com`, `msn.com`, regional variants) → `"Outlook"`; all other non-Gmail domains (corporate/SSO e.g. `cognizant.com`) → `"Outlook365"`. Helper: `_outlook_provider(email)` + `_CONSUMER_OUTLOOK_DOMAINS` frozenset in `email_summarizer.py`. Frontend components (`NotificationDrawer`, `TriggerToast`) build dynamic deep-links to the specific draft: Gmail → `https://mail.google.com/mail/u/0/#search/{subject}`; Outlook consumer → `https://outlook.live.com/mail/0/drafts/id/{draft_id}`; Outlook365 → `https://outlook.cloud.microsoft/mail/drafts/id/{draft_id}`. Fallback (no draft_id): consumer → generic drafts folder; Outlook365 → `https://outlook.cloud.microsoft/mail/drafts?deeplink=mail%2F0%2Fdrafts`. Constants: `GMAIL_SEARCH_BASE_URL`, `OUTLOOK_DRAFT_BASE_URL`, `OUTLOOK_DRAFTS_URL`, `OUTLOOK_M365_DRAFT_BASE_URL`, `OUTLOOK_M365_DRAFTS_URL` in `frontend/lib/constants.ts`. `NotificationItem` type includes `draft_id?: string` and `provider?: "Gmail" | "Outlook" | "Outlook365"`.
